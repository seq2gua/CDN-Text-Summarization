...
def create_batch(sources, targets):
    ...
    return batch
'''
def create_batch(sources, targets,bert_embedding):
    batch = Batch()
    batch.encoder_inputs_length = [len(source) for source in sources]
    # len(target) + 1 because of one <EOS>
    batch.decoder_targets_length = [len(target) + 1 for target in targets]
    ...
    for bert in bert_embedding:
        pad = [zero_pad] * (max_target_length - len(bert))
...
def get_batches(sources_data, bert_embedding_data,targets_data,batch_size):
    ...
    def gen_next_samples():
        for i in range(0, data_len, batch_size):
            yield sources_data[i:min(i + batch_size, data_len)], bert_embedding_data[i:min(i + batch_size, data_len)],targets_data[i:min(i + batch_size, data_len)]
...
if __name__ == '__main__':
    ...
    keep_rate = 0.6
    batch_size = 32
    a = pickle.load(open('bert_embedding.txt', 'rb'))
